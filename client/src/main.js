...
function isMobile() {
  ...
  const result = hasTouch && (isSmallScreen || isMobileUserAgent);
  console.log('[isMobile] Result:', result, { hasTouch, isSmallScreen, isMobileUserAgent, width: window.innerWidth });
  return result;
}

function createTouchControls() {
  console.log('[TouchControls] Starting...');

  // Clean up existing
  ['touch-overlay', 'joystick-knob', 'mobile-block-selector'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.remove();
  });

  const isMob = isMobile();
  console.log('[TouchControls] Is mobile:', isMob);

  // Prevent browser scrolling
  document.body.style.overflow = 'hidden';
  document.body.style.touchAction = 'none';

  // Disable canvas touch
  const app = document.getElementById('app');
  const canvas = document.querySelector('canvas');
  if (app) { app.style.pointerEvents = 'none'; app.style.touchAction = 'none'; }
  if (canvas) { canvas.style.pointerEvents = 'none'; canvas.style.touchAction = 'none'; }
  ...
  overlay.style.cssText = `
    ...
    z-index: 2147483647; touch-action: none; pointer-events: auto;
    display: ${isMob ? 'block' : 'none'}; background: transparent;
  `;
  document.body.appendChild(overlay);
  ...
  knob.style.cssText = `
    position: fixed; bottom: 155px; left: 75px; width: 40px; height: 40px;
  ...
  // Labels
  ['DRAG TO MOVE', 'DRAG TO LOOK', 'TAP TO PLACE'].forEach((text, i) => {
    const label = document.createElement('div');
    label.style.cssText = `
      position: fixed; color: white; font-family: monospace; font-size: 12px;
      text-shadow: 1px 1px 2px black; pointer-events: none; z-index: 2147483647;
    `;
    if (i === 0) { label.textContent = text; label.style.bottom = '230px'; label.style.left = '50px'; label.style.fontSize = '14px'; label.style.fontWeight = 'bold'; }
    else if (i === 1) { label.textContent = text; label.style.top = '15px'; label.style.right = '20px'; }
    else { label.textContent = text; label.style.bottom = '120px'; label.style.right = '30px'; }
    overlay.appendChild(label);
  });

  // Block selector
  const selector = document.createElement('div');
  selector.id = 'mobile-block-selector';
  selector.style.cssText = `
    position: fixed; bottom: 15px; left: 50%; transform: translateX(-50%);
    display: flex; gap: 8px; background: rgba(0,0,0,0.85); padding: 10px;
    border-radius: 12px; z-index: 2147483647; pointer-events: auto; touch-action: none;
  `;
  ...
  blockTypes.forEach((type) => {
    ...
    btn.style.cssText = `
      ...
      background: #${(CONFIG.BLOCK_TYPES[type.toUpperCase()]?.color || 0x696969).toString(16).padStart(6, '0')};
      border: 2px solid transparent; border-radius: 8px; flex-shrink: 0;
    `;
    btn.addEventListener('touchstart', (e) => {
  ...
  // Touch zones - each zone is a separate element
  const zones = [
    { id: 'joystick-zone', left: 0, bottom: 0, width: '50%', height: '50%' },
    { id: 'camera-zone', right: 0, top: 0, width: '50%', height: '60%' },
    { id: 'block-zone', right: 0, bottom: 0, width: '50%', height: '50%' }
  ];
  ...
  zones.forEach(z => {
    ...
    el.id = z.id;
    el.style.cssText = `
      position: fixed; ${z.left ? 'left: 0;' : ''} ${z.right ? 'right: 0;' : ''}
      ${z.top ? 'top: 0;' : ''} ${z.bottom ? 'bottom: 0;' : ''}
  ...
  // Event handlers
  document.getElementById('joystick-zone').addEventListener('touchstart', handleJoystickStart, { passive: false });
  document.getElementById('joystick-zone').addEventListener('touchmove', handleJoystickMove, { passive: false });
  document.getElementById('joystick-zone').addEventListener('touchend', handleJoystickEnd, { passive: false });
  document.getElementById('camera-zone').addEventListener('touchstart', handleCameraStart, { passive: false });
  document.getElementById('camera-zone').addEventListener('touchmove', handleCameraMove, { passive: false });
  document.getElementById('camera-zone').addEventListener('touchend', handleCameraEnd, { passive: false });
  document.getElementById('block-zone').addEventListener('touchstart', handleBlockStart, { passive: false });
  document.getElementById('block-zone').addEventListener('touchend', handleBlockEnd, { passive: false });
